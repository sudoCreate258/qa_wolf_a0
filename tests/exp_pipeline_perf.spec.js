...
import { withAveragedMetrics, calcImproMet, printImprov, printCompTable } from '../src/performance_utils.js';
...
const ITERATIONS = 5; // Run each test 5 times and average results
...
    test('Summary: Baseline vs Stage 1 Across Browsers (Averaged)', async ({ page }) => {
        console.log(`[Config] THRESHOLD: ${THRESHOLD}, Tolerance: ${MEMORY_TOLERANCE_FACTOR}, Iterations: ${ITERATIONS}`);
        console.log('\n' + '='.repeat(80));

        // 1. Run Baseline (Averaged)
        try { 
            baselineResult = await withAveragedMetrics('Baseline', baselineRun, ITERATIONS, page);
...
        // 2. Run Stage 1 (Averaged)
        try{
            // Note: Since stage1Run takes an optional 'iterations' argument, we pass 5 here
            stage1Result = await withAveragedMetrics('Stage 1', stage1Run, ITERATIONS, page, 5);
...
        // Assert: The memory delta of the optimized version should be less than the baseline
